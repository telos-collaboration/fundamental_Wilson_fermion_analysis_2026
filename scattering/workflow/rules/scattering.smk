...
rule plot_luescher_results:
    input:
        script="src/src_py/plot_fit_scatter.py",
...
rule phase_shift_plots:
    input:
        script="src/src_py/phase_shift_plots.py",
        h5file="../data_assets/scattering/isospin1_fit_scatter.hdf5",
    output:
...
rule crossection_plots:
    input:
        script="src/src_py/cross_section_plot.py",
        h5file="../data_assets/scattering/isospin1_fit_scatter.hdf5",
...
rule luescher_tables:
    input:
        script="src/src_py/result_tables.py",
        h5file="../data_assets/scattering/isospin1_fit_scatter.hdf5",
    output:
...
rule parse_flows:
    input:
        script="src/src_py/package_flows.py",
    output:
...
rule plot_meson_finite_volume:
    input:
        script="src/scripts_julia/finite_volume.jl",
...
rule ensemble_table:
    input:
        script="src/scripts_julia/ensemble_tables.jl",
...
rule phase_shift_csv:
    input:
        script="src/scripts_julia/phase_shift_table.jl",
        h5file_in="../data_assets/scattering/isospin1_fit_scatter.hdf5",
        julia_instantiated="../intermediary_data/julia_ready",
    output:
        table = "../intermediary_data/{pattern}.csv",
    conda:
        "../envs/scattering.yml"
    shell:
...
rule phase_shift_tex:
    input:
        script="src/scripts_julia/phase_shift_tex.jl",
        table = "../intermediary_data/{pattern}.csv",
        metadata = "../metadata/scattering/fit_scatter_input.csv",
        julia_instantiated="../intermediary_data/julia_ready",
    output:
