...
def fit_all_files(infile,outfile,parameterfile):
    ...
    for row in tqdm((reader), total=lines , desc="Fit eigenvalues", disable=False):
        ...
        if "Cpi" in fid[op.join(ensemble,p)].keys():
            ...
            E, a, chi2, dof = fit_correlator_without_bootstrap(cor_pi,T,3,T//2-2,Nmax,False,plotname,plotdir,plotting,printing)
        ...
        if op.join(ensemble,p0,"T1/E") not in f.keys():
            ...
            E, a, chi2, dof = fit_correlator_without_bootstrap(cor_T1,T,3,T//2-2,Nmax,False,plotname,plotdir,plotting,printing)
        ...
        if "E" in fid[op.join(ensemble,p)].keys():
            ...
            E, a, chi2, dof = fit_correlator_without_bootstrap(cor_E,T,3,T//2-2,Nmax,False,plotname,plotdir,plotting,printing)
        ...
        if "B1" in fid[op.join(ensemble,p)].keys():
            ...
            E, a, chi2, dof = fit_correlator_without_bootstrap(cor_B1,T,3,T//2-2,Nmax,False,plotname,plotdir,plotting,printing)
