...
def ylabel_f(yaxis):
    if yaxis == "p3cotPS_prime":
        ...
        return r"$p^3\, \cot(\delta)/E_{cm}/m_\pi^2$"
    elif yaxis == "sigma_prime":
...
def plot_p3cot_PS_p2_non_res(h5file,beta,m0,xaxis="p2star_prime",yaxis="p3cotPS_prime",fit_model=None,outname=None,show=False):
    fit = fit_model != None
    plt.rcParams['figure.figsize'] = [10, 6]
    fontsize = 14
    font = {'size'   : fontsize}
    ...
    if fit:
        ...
        xarr = np.linspace(xlim[0], xlim[1], 40)
    ...
    xticks = np.linspace(0,0.3,7)
    ax.set_xticks(xticks, [r"$%1.2f$"%x for x in xticks])
...
def plot_PS_s_non_res(h5file,beta,m0,xaxis="p2star_prime",yaxis="p3cotPS_prime",fit_model=None,outname=None,show=False):
    fit = fit_model != None
    plt.rcParams['figure.figsize'] = [10, 6]
    fontsize = 14
    font = {'size'   : fontsize}
...
def plot_PS_s_res(h5file,beta,m0,xaxis="p2star_prime",yaxis="p3cotPS_prime",fit_model=None,outname=None,show=False):
    fit = fit_model != None
    plt.rcParams['figure.figsize'] = [10, 6]
    fontsize = 14
    font = {'size'   : fontsize}
    ...
    if fit:
        ...
        xarr = np.linspace(xlim[0], xlim[1], 80)
        # for i in  range(len(x_nf_m)):
        #     ax.scatter(x_nf_m[i],y_nf_m[i], color = "grey", ls = pf.ls(*plot_args_nf[i]), marker = pf.marker(*plot_args_nf[i]))#, s = 10*pf.ms(*plot_args_nf[i]))   #, label = "|P|=%i, NL=%i"%(d2s[i],N_Ls[i])
        #     sorted_indices = np.argsort(x_nf_s[i])
        #     ax.plot(x_nf_s[i][sorted_indices][math.floor(length*(1-num_perc)/2):math.ceil(length*(1+num_perc)/2)],delete_steps(y_nf_s[i][sorted_indices])[math.floor(length*(1-num_perc)/2):math.ceil(length*(1+num_perc)/2)], color = "grey", ls = pf.ls(*plot_args_nf[i]))
...
def plot_p3cot_PS_s_res(h5file,beta,m0,xaxis="p2star_prime",yaxis="p3cotPS_prime",fit_model=None,outname=None,show=False):
    fit = fit_model != None
    plt.rcParams['figure.figsize'] = [10, 6]
    fontsize = 14
    font = {'size'   : fontsize}
...
if __name__ == "__main__":
    ...
    if not fit:
        ...
        # plot_p3cot_PS_p2_non_res(h5file, 6.9, -0.92, "p2star_prime", "p3cotPS_prime", fm.ERE_0_model)
        # # plot_p3cot_PS_p2_non_res(h5file, 6.9, -0.92, "p2star_prime", "p3cotPS_prime", fm.ERE_1_model)
        # plot_p3cot_PS_s_res(h5file, 7.05, -0.867, "s_prime", "p3cotPS_Ecm_prime", fm.BW_I_model)
        # # plot_p3cot_PS_s_res(h5file, 7.05, -0.867, "s_prime", "p3cotPS_Ecm_prime", fm.BW_II_model)
        # plot_PS_s_non_res(h5file, 6.9, -0.92, "p2star_prime", "PS", fm.ERE_0_model)
        # # plot_PS_s_non_res(h5file, 6.9, -0.92, "s_prime", "PS", fm.ERE_1_model)
        # plot_PS_s_res(h5file, 7.05, -0.867, "s_prime", "PS", fm.BW_I_model)

        # plot_PS_s_res(h5file, 7.05, -0.867, "s_prime", "PS", fm.BW_II_model)
        ...
        plot_any(h5file, 6.9, -0.92, "p2star_prime", "PS", fm.ERE_0_model)
